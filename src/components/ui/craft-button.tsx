...
function CraftButtonLabel({ children, className }: CraftButtonLabelProps) {
  return (
    <span
      className={cn(
        'group-hover:text-foreground relative z-2 transition-colors duration-500 dark:text-foreground',
        className,
      )}
    >
      {children}
...
function CraftButtonIcon({ children, className }: CraftButtonIconProps) {
  ...
  return (
    <span className={cn('relative z-1', iconSize, className)}>
      ...
      <span
        className={cn(
          'bg-background text-accent group-hover:bg-accent group-hover:text-background relative z-2 flex items-center justify-center rounded-full transition-all duration-500',
          iconSize,
        )}
      >
        {children}
  ...
  return (
    <CraftButtonContext.Provider value={{ size }}>
      <Button
        ...
        asChild={asChild}
        className={cn(
          'group bg-accent hover:bg-background dark:hover:border-accent/30 relative cursor-pointer overflow-hidden rounded-full duration-500 hover:shadow-md dark:border dark:border-transparent',
          className,
        )}
        {...rest}
      >
